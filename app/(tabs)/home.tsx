...
const HomeScreen = () => {
  ...
  const fetchUserData = async () => {
    try {
      ...
      const response = await fetch(
        `http://54.207.207.242:3000/users/email/${storedEmail}`,
        {
          method: "GET",
          headers: {
      ...
      if (!response.ok) {
        Alert.alert(
          "Erro",
          data.message || "Erro ao carregar os dados do usuário"
        );
      } else {
        setUser(data);
        setUserId(data.id);
  ...
  const handleImagePick = async () => {
    ...
    if (status !== "granted") {
      ...
      Alert.alert(
        ...
        "Você precisa conceder acesso à galeria para selecionar uma imagem."
      );
      return;
    }
  ...
  const handleImageUpload = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `http://54.207.207.242:3000/users/${userId}/avatar`,
        {
          method: "PUT",
          headers: {
    ...
    <View style={styles.container}>
      ...
      <View style={styles.motivationalBox}>
        <Text style={styles.motivationalText}>
          Você está mais perto de alcançar seus objetivos!
        </Text>
      </View>

      <View style={styles.consultationBox}>
        <Text style={styles.consultationText}>
          Próxima consulta: 20/03/2025
        </Text>
        <FontAwesome
          name="check-circle"
          size={24}
