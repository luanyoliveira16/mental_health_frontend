...
const CodeVerificationRegister: React.FC = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      console.log("Enviando dados para a API:", { email, token: code }); // Log antes de enviar os dados

      const response = await axios.post(
        "http://54.207.207.242:3000/auth/validate-signup",
        { email, token: code } // Envia o token como o código inserido
      );
      ...
      if (response.data.success) {
        ...
        Alert.alert(
          "Erro",
          response.data.message || "Erro ao verificar código"
        );
      }
    } catch (error) {
